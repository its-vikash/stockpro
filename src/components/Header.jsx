...
export default function Header(){
  ...
  return (
    <header className="header-main bg-gradient-to-r from-dark to-primary text-white shadow-lg sticky top-0 z-50">
      ...
      <div 
        ref={mobileMenuRef}
        className={`md:hidden bg-gradient-to-b from-dark to-primary transition-all duration-300 ease-in-out ${mobileMenuOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0 overflow-hidden'}`}
      >
        <div className="container mx-auto px-4 py-4 space-y-4">
          <div className="relative mb-4" ref={searchRef}>
            <div className="flex items-center gap-2 bg-white/10 rounded-full px-4 py-2 transition-all duration-300 hover:bg-white/15 focus-within:ring-2 ring-accent2">
              <Search className="h-4 w-4 text-accent1" />
              <input 
                className="bg-transparent outline-none placeholder-white/70 text-white w-full" 
                value={query} 
                onChange={onSearch} 
                placeholder="Search derivatives, symbols..." 
              />
          ...
          <nav>
            <ul className="space-y-2">
              <li>
                <Link 
                  to="/" 
                  className={`block px-4 py-2 rounded-lg transition-all duration-300 hover:bg-white/10 ${loc.pathname==='/' ? 'bg-white/10 text-accent2' : ''}`}
              ...
              <li>
                <Link 
                  to="/derivatives" 
                  className={`block px-4 py-2 rounded-lg transition-all duration-300 hover:bg-white/10 ${loc.pathname==='/derivatives' ? 'bg-white/10 text-accent2' : ''}`}
                  onClick={() => setMobileMenuOpen(false)}
                >
                  Derivatives
                </Link>
              ...
              <li>
                <Link 
                  to="/options-chain" 
                  className={`block px-4 py-2 rounded-lg transition-all duration-300 hover:bg-white/10 ${loc.pathname==='/options-chain' ? 'bg-white/10 text-accent2' : ''}`}
                  onClick={() => setMobileMenuOpen(false)}
                >
                  Options Chain
                </Link>
              ...
              <li>
                <Link 
                  to="/news" 
                  className={`block px-4 py-2 rounded-lg transition-all duration-300 hover:bg-white/10 ${loc.pathname==='/news' ? 'bg-white/10 text-accent2' : ''}`}
                  onClick={() => setMobileMenuOpen(false)}
                >
                  News
                </Link>
          ...
          <div className="pt-4 border-t border-white/10">
            {isAuthenticated ? (
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <div className="w-8 h-8 rounded-full bg-gradient-to-r from-accent1 to-accent2 text-dark font-bold flex items-center justify-center">
                ...
                <button 
                  ...
                  className="px-3 py-1 rounded-lg bg-white/10 hover:bg-white/20 transition-colors duration-300 flex items-center"
                >
              ...
              <div className="flex items-center gap-2 justify-center">
                <Link 
                  to="/login" 
                  className="px-4 py-2 rounded-lg bg-white/10 hover:bg-white/20 transition-colors duration-300"
                  onClick={() => setMobileMenuOpen(false)}
                >
                  Login
                </Link>
                <Link 
                  to="/register" 
                  className="px-4 py-2 rounded-lg bg-accent2 text-dark hover:bg-accent1 transition-colors duration-300"
                  onClick={() => setMobileMenuOpen(false)}
                >
                  Register
                </Link>
